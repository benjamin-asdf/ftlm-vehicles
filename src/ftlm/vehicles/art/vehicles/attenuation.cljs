...
(defmethod lib/setup-version :attenuation
  ...
  (let
      [controls (:controls state)
                  ...
                  {:activations #js []
                   ...
                   (ac/->directed-graph-with-geometry
                    ...
                    (ac/lin-gaussian-geometry-wrap
                     {:amplitude (:connectivity-amplitude controls)
                      :n-neurons n-neurons
                      :std-deviation (:connectivity-std-deviation controls) }))})
       ...
       (into {}
             (map (juxt :id
                        #(ac/->rand-projection n-neurons (:stimulus-projection-density controls)))
                  stimuli))
