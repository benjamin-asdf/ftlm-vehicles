...
(defmethod lib/setup-version :excitability
  ...
  (let
      [controls (:controls state)
       ...
       n-area (-> n-area
                  (assoc
                   ...
                   {:activations #js []
                    :inhibition-model
                    (comp
                     (rand-cap-k-threshold-device [(* 0.1 n-neurons) (* 0.05 n-neurons)])
               ...
               (map-indexed
                (fn [stim-idx stimulus]
                  [(:id stimulus)
                   ...
                   ;;                sector-lenght))
                   ;;      0.1)))
                   (ac/->projection n-neurons (constantly 0.01))])
                stimuli)))
       ...
       eye-movement
       (->eye-movement-effectors {:n-neurons n-neurons :eye eye-ball :visual-field visual-field :n-area n-area})
       state (-> state
      ...
      (->
       ...
       (assoc-in [:on-update-map :eye-movement] (:update eye-movement))
