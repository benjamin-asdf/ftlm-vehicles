(ns ftlm.vehicles.art.vehicles.pulling-stimuli
  (:require
...
(defn ->visual-field
  ...
  (->
   ...
   ;; (lib/live [:fade (lib/->fade-pulse-2 6.0 :stroke)])
   ))
...
(defmethod lib/setup-version :pulling-stimuli
  [state]
  (let
      [controls (:controls state)
       ...
       n-area (->
               ...
               (assoc :ac-area
                      {:activations #js []
                       ...
                       :attenuation-malus-factor 0.8
                       :excitability-growth 1.6
                    ...
                    (lib/live [:breath
                               (elib/->tiny-breath
                                {:speed 1
                                 ...
                                 :stop 1.075})])
                    (assoc :particle? true)
                    (assoc :kinetic-energy 1.3))))
             ;; (take 21 (shuffle (keys controls/color-map)))
       ...
       ;; perspective-flower  (->perspective-flower {:n-neurons n-neurons})
       ...
       sensoric-field
       (na/->sensoric-field [631 400])

       append-stimuli-inputs
       (fn [s]
         (append-inputs s
                        (keep stimulus->recptive-field
                              (:stimuli ((lib/entities-by-id s)
                                         (:id sensoric-field))))))
         ...
         (fn [e s _]
           {:updated-state
            (let [s (update s :neuron-tick (fnil inc 0))
                  wipe-inputs? (zero? (mod (:neuron-tick s) 10))
                  append-inputs?
                  (when-not wipe-inputs?
                    (even? (:neuron-tick s)))]
              (cond-> s
                (odd? (:neuron-tick s)) neuron-tick
                wipe-inputs? (update-in [:eid->entity (:id n-area) :ac-area] ac/set-input #js[])
