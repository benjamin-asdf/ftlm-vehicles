...
(defn cycling-cap-k-threshold-device
  [numbers]
  (let [next-numbers (atom (cycle numbers))]
...
(defmethod lib/setup-version :contrast
  ...
  (let
    [controls (:controls state)
     ...
     n-area
       (na/->neuronal-area-ac-1
         {:grid-width 20
          :layer-config {:height 20
                         :layers [:sensor-input :contrast
                                  :motor-output]}
          :n-neurons n-neurons
          :spacing 15
                ...
                (assoc
                  :ac-area
                    {:activations #js []
                     ...
                     :attenuation-malus-factor 0.2
                     :density 0.1
                     :excitability-decay 0.1
                     :excitability-growth 10.0
                     :inhibition-model
                     ...
                     ;;  n-neurons
                     ;;  (ac/lin-gaussian-geometry {:amplitude 1.0 :std-deviation 50}))
                          ...
                          (for [n neurons]
                            [n
                             (lib/->hsb
                              (:red
                               controls/color-map))]))))
                    (-> eye-movement :effector-projections))
